...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    """
    _LOGGER.info(
        "Afregistrerer Braendstofpriser integration for entry: %s", entry.entry_id
    )
    ...
    try:
        ...
        if unload_ok:
            ...
            _LOGGER.info(
                "Braendstofpriser integration afregistreret for entry: %s",
                entry.entry_id,
            )
        else:
            _LOGGER.warning(
                "Kunne ikke afregistrere Braendstofpriser integration for entry: %s",
                entry.entry_id,
            )
        return unload_ok
    except Exception as err:
